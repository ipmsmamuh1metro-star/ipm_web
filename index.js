...
const MOCK_USERS = [
  { email: 'vaalh', password: '56964321', name: 'Vaalh', role: 'admin' },
  ...
  { email: 'Merryn Berlian', password: 'm3b7i29', name: 'Merryn Berlian', role: 'admin' }
];
...
function renderHome() {
  ...
  // News grid - using compact card design
  ...
  grid.innerHTML = state.news.slice(0, 3).map(n => {
    ...
    return `
    <div class="news-card" onclick="showNewsDetail(${n.id}, event)" style="cursor: pointer;">
      <div class="news-image">
        <img src="${n.image_url || 'https://via.placeholder.com/400x225?text=IPM'}" alt="${n.title}" onerror="this.src='https://via.placeholder.com/400x225?text=IPM'">
      </div>
...
function renderNewsList() {
  ...
  grid.innerHTML = paginated.map((n, idx) => {
    ...
    const excerpt = n.content.substring(0, 120) + '...';
    ...
    <div class="news-card" onclick="showNewsDetail(${n.id}, event)" style="cursor: pointer;">
      ...
      <div class="news-content">
        ...
        <div class="news-meta">
          ...
          <a class="news-link">Selengkapnya</a>
        </div>
      </div>
    </div>`;
  }).join('');
...
// ── INTERACTIVE HELPERS ──
function togglePasswordVisibility(inputId, eventObj) {
  const input = document.getElementById(inputId);
  const ev = eventObj || (typeof event !== 'undefined' ? event : window.event);
  const button = ev ? ev.target.closest('.password-toggle') : null;
  if (!button) return;
...
function doLoginMember(eventObj) {
  ...
  setTimeout(() => {
    // First check synced database members from localStorage
    let members = JSON.parse(localStorage.getItem('members') || '[]');
    let member = members.find(m => m.fullname && m.fullname.toLowerCase() === username && m.password === pass);
    ...
    if (member) {
      ...
      displayUsername = member.fullname.toLowerCase();
      displayAvatar = member.fullname.charAt(0).toUpperCase();
      
      // Update member dashboard elements
      document.getElementById('member-name').textContent = member.fullname;
      document.getElementById('member-welcome-name').textContent = member.fullname.split(' ')[0];
      document.getElementById('member-avatar').textContent = displayAvatar;
      document.getElementById('member-topbar-title').textContent = 'Dashboard';
    } else {
      // Fallback to MOCK_MEMBERS
      const mockMember = MOCK_MEMBERS.find(m => m.username.toLowerCase() === username && m.password === pass);
      if (mockMember) {
        displayName = mockMember.name;
        displayUsername = mockMember.username;
        displayAvatar = mockMember.name.charAt(0).toUpperCase();
        
        // Update member dashboard elements
        document.getElementById('member-name').textContent = mockMember.name;
        document.getElementById('member-welcome-name').textContent = mockMember.name.split(' ')[0];
        document.getElementById('member-avatar').textContent = displayAvatar;
        document.getElementById('member-topbar-title').textContent = 'Dashboard';
      } else {
    ...
    // Store member info expected by initMemberDashboard
    state.currentMember = {
      name: displayName,
      username: displayUsername,
      avatar: displayAvatar
    };
    localStorage.setItem('currentMember', JSON.stringify(state.currentMember));
...
function switchPanel(name, el) {
  // STEP 1: Hide ALL panels explicitly
  ...
  // STEP 2: Remove active state from ALL sidebar links
  document.querySelectorAll('.sidebar-link').forEach(l => l.classList.remove('active'));
  
  // STEP 3: Find and activate the selected panel
  const panel = document.getElementById('panel-' + name);
  if (!panel) {
    console.error('❌ Panel tidak ditemukan:', 'panel-' + name);
    return;
  }
  
  // Force display the active panel
  panel.classList.add('active');
  panel.style.display = 'block';
  panel.style.visibility = 'visible';
  panel.style.position = 'relative';
  
  // STEP 4: Mark the clicked link as active
  if (el) {
    el.classList.add('active');
  }
  
  // STEP 5: Update topbar title
  const titles = {
    'dashboard': 'Dashboard',
    'news-admin': 'Kelola Berita',
    'ach-admin': 'Kelola Prestasi',
    'calendar-admin': 'Kalender Acara',
    'member-bidang': 'Bidang Anggota',
    'manage-members': 'Kelola Anggota',
    'info-data': 'Info Data',
    'edukasi-admin': 'Edukasi',
    'dokumentasi-admin': 'Dokumentasi'
  };
  ...
  // STEP 6: Load content based on panel
  console.log(`✨ Menampilkan panel: ${name}`);
  
  if (name === 'news-admin') renderAdminNews();
  else if (name === 'ach-admin') renderAdminAch();
  else if (name === 'calendar-admin') renderAdminCalendar();
  else if (name === 'member-bidang') renderAdminMemberBidang();
  else if (name === 'dashboard') refreshAdminData();
  else if (name === 'manage-members') renderAdminMembers();
  else if (name === 'info-data') renderInfoData();
  else if (name === 'edukasi-admin') renderAdminEdukasi();
  else if (name === 'dokumentasi-admin') renderAdminDokumentasi();
...
// ── RENDER ADMIN MEMBERS (MANAGE MEMBERS) ──
function renderAdminMembers() {
  const tbody = document.querySelector('#manage-members-table tbody');
  tbody.innerHTML = state.members.map(m => `
    <tr>
      <td><div class="td-title">${m.fullname || m.name || 'N/A'}</div></td>
      <td><span style="color:var(--muted); font-size:.9rem;">${m.email || 'N/A'}</span></td>
      <td><span style="background:rgba(0,128,0,0.1); padding:.35rem .75rem; border-radius:6px; font-size:.85rem; color:var(--gold); font-weight:600;">${m.bidang || 'Umum'}</span></td>
      <td style="color:var(--muted); font-size:.9rem;">${m.registered_at ? formatDate(m.registered_at) : (m.created_at ? formatDate(m.created_at) : '-')}</td>
      <td><span class="status-badge ${m.status === 'aktif' ? 'badge-green' : 'badge-yellow'}">${m.status || 'aktif'}</span></td>
      <td><div class="action-btns">
        <button class="btn-icon edit" title="Edit" onclick="editMember(${m.id})">✏️</button>
        <button class="btn-icon delete" title="Hapus" onclick="removeAdminMember(${m.id})">🗑️</button>
      </div></td>
    </tr>`).join('');
}
...
function renderInfoData() {
  console.log('[Info] Menampilkan Data');
  // Auto-populate current data if exists in state
  const infoAktif = document.getElementById('info-aktif');
  const infoPrestasi = document.getElementById('info-prestasi');
  const infoTahun = document.getElementById('info-tahun');
  const infoProgram = document.getElementById('info-program');
  
  if (infoAktif) infoAktif.value = state.info_aktif || state.members.length || 0;
  if (infoPrestasi) infoPrestasi.value = state.info_prestasi || state.achievements.length || 0;
  if (infoTahun) infoTahun.value = state.info_tahun || 2024;
  if (infoProgram) infoProgram.value = state.info_program || state.programs.length || 0;
}

// ── RENDER ADMIN EDUKASI ──
function renderAdminEdukasi() {
  const tbody = document.querySelector('#edukasi-admin-table tbody');
  tbody.innerHTML = (state.edukasi || []).map(e => `
    <tr>
      <td>${renderThumb(e.image_url || '📚', '')}</td>
      <td><div class="td-title">${e.title}</div><div class="td-sub">${e.category || 'Umum'}</div></td>
      <td><span style="background:rgba(0,128,0,0.1); padding:.35rem .75rem; border-radius:6px; font-size:.85rem; color:var(--gold); font-weight:600;">${e.category || 'Umum'}</span></td>
      <td style="color:var(--muted); font-size:.9rem;">${e.created_at ? formatDate(e.created_at) : 'Baru'}</td>
      <td><div class="action-btns">
        <button class="btn-icon edit" title="Edit" onclick="editEdukasi(${e.id})">✏️</button>
        <button class="btn-icon delete" title="Hapus" onclick="deleteEdukasi(${e.id})">🗑️</button>
      </div></td>
    </tr>`).join('') || '<tr><td colspan="5" style="text-align:center; color:var(--muted); padding:2rem;">Belum ada materi edukasi</td></tr>';
}

// ── RENDER ADMIN DOKUMENTASI ──
function renderAdminDokumentasi() {
  const tbody = document.querySelector('#dokumentasi-admin-table tbody');
  tbody.innerHTML = (state.dokumentasi || []).map(d => `
    <tr>
      <td><div class="td-title">${d.title}</div></td>
      <td><a href="${d.link}" target="_blank" style="color:var(--gold); text-decoration:none; font-weight:600;">🔗 Lihat Link</a></td>
      <td><span style="background:rgba(255,255,0,0.1); padding:.35rem .75rem; border-radius:6px; font-size:.85rem; color:#a16207; font-weight:600;">${d.category || 'Dokumen'}</span></td>
      <td><div class="action-btns">
        <button class="btn-icon edit" title="Edit" onclick="editDokumentasi(${d.id})">✏️</button>
        <button class="btn-icon delete" title="Hapus" onclick="deleteDokumentasi(${d.id})">🗑️</button>
      </div></td>
    </tr>`).join('') || '<tr><td colspan="4" style="text-align:center; color:var(--muted); padding:2rem;">Belum ada dokumentasi</td></tr>';
}
    ...
    newsSection.innerHTML = `
      ...
      <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(280px, 1fr)); gap:1.5rem; margin-bottom:2.5rem;">
        ${state.news && state.news.length > 0 ? state.news.slice(0, 3).map(n => `
          <div style="background:var(--white); border:1.5px solid rgba(0,128,0,0.08); border-radius:12px; overflow:hidden; transition:all .3s; cursor:pointer;" onclick="switchMemberPanel('news', document.querySelector('.sidebar-link[onclick*=\\'news\\']'))">
...
function renderCalendar() {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    dayDiv.addEventListener('click', () => {
      document.querySelectorAll('#calendar-days .calendar-day').forEach(d => d.classList.remove('selected'));
...
async function saveMember() {
  ...
  try {
    const memberData = {
      ...
      bidang,
      status,
      registered_at: memberId ? (state.members.find(m => m.id === parseInt(memberId))?.registered_at || new Date().toISOString().split('T')[0]) : new Date().toISOString().split('T')[0]
    };
...
async function removeAdminMember(memberId) {
  const member = state.members.find(m => m.id === memberId);
  if (!member) {
    showToast('❌ Anggota tidak ditemukan', 'error');
    return;
  ...
  if (!confirm(`Yakin hapus anggota "${member.fullname}"?`)) return;
  
  try {
    console.log(' [DELETE] members ID', memberId);
    await sbDelete('members', memberId);
    console.log('✅ [DELETE SUCCESS] members ID', memberId);
...
window.addEventListener('DOMContentLoaded', async () => {
  // Initialize all admin panels - hide them all first
...
function initializeAdminPanels() {
  ...
  // Show only the dashboard panel
  const dashboardPanel = document.getElementById('panel-dashboard');
  if (dashboardPanel) {
    dashboardPanel.classList.add('active');
    dashboardPanel.style.display = 'block';
    dashboardPanel.style.visibility = 'visible';
  }
  
  // Mark dashboard sidebar link as active
  const sidebarLinks = document.querySelectorAll('.sidebar-link');
  sidebarLinks.forEach(link => link.classList.remove('active'));
  
  const dashboardLink = document.querySelector('.sidebar-link[onclick*="dashboard"]');
  if (dashboardLink) {
